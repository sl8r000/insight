...
def fetch_dataset(stack_overflow_client):
    ...
    for user_id in user_ids:
        user_answers = stack_overflow_client.users.ids(user_id).answers.get(pagesize=50)
        random_answers = stack_overflow_client.answers.get(pagesize=50)
        all_answers = user_answers + random_answers
